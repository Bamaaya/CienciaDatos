...
#A cada función que involucra solo números añadí un ciclo que se encarga de eliminar los nans con math
...
# Función moda():
    ...
    # Finalmente, retorna dos listas:
        ...
        #Una lista con listas pareadas, las cuales contienen cada variable y numero de repeticiones en sus indices 0 y 1 respectivamente
...
def moda(valores):
    ...
    listaCantidades = []
    for m in range(len(listaVariables)):    #Ciclo usado para crear las listas pareadas de listaCantidades
        for n in range(maximo+1):
            if n in Cantidades:              
                lista = []
                lista.append(listaVariables[n])     #En cada ciclo se le añade el índice respectivo de listaVariables y Cantidades
                lista.append(Cantidades[n])
                listaCantidades.append(lista)       #Finalmente se añade la lista a listaCantidades
...
# Función MAD():
    ...
    # Se calcula la mediana usando la función de la mediana previamente realizada
...
conjunto =[0,0,5,6,7]
datos = ["perro", "perro", "perro", "gato", "perro", "gato", "piton", "piton", "piton","piton"]

promedio2 = mediaAritmetica(lista)

mediana = mediana(conjunto)

rango = rango(lista)

desvEst = desvEst(numeros)

var = varianza(numeros)

percentiles = percentil(lista)
